...
const Background: React.FC<BackgroundProps> = ({
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`w-full h-screen absolute top-0 left-0 flex items-center justify-center bg-black/10 text-gray-500 text-sm ${className}`}
        style={{ mixBlendMode }}
      >
        WebGL not supported
      </div>
  ...
  return (
    <div ref={containerRef} className={`w-full h-screen absolute top-0 left-0 ${className}`} style={{ mixBlendMode }} />
  );
};
