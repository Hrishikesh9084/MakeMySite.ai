...
import { BotIcon, EyeIcon, Loader, SendIcon, UserIcon, } from 'lucide-react';
...
const Sidebar = ({ isMenuOpen, project, setProject, isGenerating, setIsGenerating }: SidebarProps) => {
    ...
    const handleRollback = async (versionId: string) => {
        try {
            ...
            console.log("Error rolling back version", error)
        }
    }
    ...
    const handleRevision = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const {data} = await api.post(`/api/project/version/${project.id}`, {message: input})
            fetchProject();
            toast.success(data.message);
            setInput('');
            ...
            console.log("Error requesting revision", error.message)
            clearInterval(interval);
        }        
    }
    ...
    return (
        <div className={`h-full sm:max-w-sm rounded-xl  border-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
            <div className='flex flex-col h-full'>
                        ...
                        if (isMessage) {
                            ...
                            return (
                                <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                                    {!isUser && (
                                        <div className='w-8 h-8 rounded-full bg-linear-to-br from-indigo-900 to-indigo-900 flex items-center justify-center'>
                                            <BotIcon className='size-5 text-white' />
                                        </div>
                                    )}
                                    <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-8 leading-relaxed ${isUser ? "bg-linear-to-r from-indigo-900 to-indigo-900 text-white rounded-tr-none" : "rounded-tl-none bg-gray-800 text-gray-100"}`}>
                                        {msg.content}
                                    </div>
                                    {isUser && (
                                        <div>
                                    ...
                                    <div className='flex items-center justify-between'>
                                        {
                                            project.current_version_index === ver.id ? (
                                                ...
                                                : (
                                                    <button onClick={() => handleRollback(ver.id)} className='px-3 py-1 rounded-md text-xs bg-indigo-900 hover:bg-indigo-900 text-white'>Role back to this version</button>
                                                )
                                        }
                    ...
                    })}
                    {
                        isGenerating && (
                            <div className='flex items-start gap-3 justify-start'>
                                <div className='w-8 h-8 rounded-full bg-linear-to-br from-indgo-600 to-ndigo-700 flex items-center justify-center'>
                                    <BotIcon className='size-5 text-white' />
                                </div>
                                {/*  three dot loader */}
                ...
                <form onSubmit={handleRevision} className='m-3 relative'>
                    <div className='flex items-center gap-2'>
                        <textarea onChange={(e) => setInput(e.target.value)} value={input} className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all' disabled={isGenerating} rows={4} placeholder='Describe your website or request changes...' />
                        <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60'>
                            {isGenerating ? <Loader className='size-7 p-1.5 animate-spin text-white' /> : <SendIcon className='size-7 p-1.5 text-white' />}
                        </button>
                    </div>
                </form>
            </div>
